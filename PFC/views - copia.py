...
def place_list(request, tp):
    if tp == 'place':
        item_list = Place.objects.all().order_by('name')
    else:
        item_list = Place.objects.all().order_by('name').filter(tipo=tp)
...
def add_photo(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            item = form.save(commit=False)
            # item.thumbnail = str(item.image.url)
            item.save()
            # print(item.image)
            # print(item.image.url)
            # item.thumbnail = str(item.image.url).replace('.jpg', '_thumbnail.jpg')
            # item.save()
            # BASE = 'W:/PROYECTO/media/'
            # path = os.path.join(BASE, str(Photo.objects.get(pk=item.pk).image).lstrip('./'))
            # thumb = Image.open(path)
            # size = (250, 250)
            # file, ext = os.path.splitext(path)
            # thumb.thumbnail(size, Image.ANTIALIAS)
            # thumb.save(file + '_thumbnail.jpg', 'JPEG')
